...
@interface NvLiveMenuTabView ()

@property(nonatomic, strong) UIScrollView *scrollView;

@end

@implementation NvLiveMenuTabView
...
- (void)didTapped:(UITapGestureRecognizer *)gesture {
    ...
    if (self.didCanSelectIndex) {
        if (!self.didCanSelectIndex(gesture.view, index))
            return;
    }
...
- (void)moveCursorTo:(NSInteger)index withAnimation:(BOOL)animate {
    ...
    [UIView animateWithDuration:self.cursorAnimationDuration
                     animations:repositionCursorBlock
