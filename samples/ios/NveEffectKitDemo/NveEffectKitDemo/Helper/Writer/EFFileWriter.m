...
- (BOOL)startRecordWithFilePath:(NSString *)filePath {
    ...
    if (error) {
        NSLog(@"Start Record Error:%@", [error description]);
        return NO;
    }
...
- (void)appendAudioSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    [self efRunSynchronouslyOnVideoProcessingQueue:^{
        ...
        @autoreleasepool {
            ...
            if (input.readyForMoreMediaData && self->_assetWriter.status != AVAssetWriterStatusUnknown) {
                BOOL success = [input appendSampleBuffer:sampleBuffer];
                if (!success) {
                    NSLog(@"append Audio SampleBuffer Error");
                }
            } else {
...
- (void)appendPixelBuffer:(CVPixelBufferRef)pixelBuffer timelinePos:(int64_t)timelinePos {
    ...
    [self efRunSynchronouslyOnVideoProcessingQueue:^{
        ...
        if (self->_videoInput.readyForMoreMediaData && self->_assetWriter.status != AVAssetWriterStatusUnknown) {
            if (![self->_adaptor appendPixelBuffer:pixelBuffer withPresentationTime:timestamp]) {
                NSLog(@"adptor load error");
            }
        } else {
    ...
    dispatch_sync(self.videoProcessingQueue, ^{
        ...
        if (status != AVAssetWriterStatusUnknown) {
            [self.assetWriter finishWritingWithCompletionHandler:^{
                AVAssetWriterStatus status1 = self.assetWriter.status;
                if (status1 != AVAssetWriterStatusUnknown) {
...
- (BOOL)setupAssetWriterVideoInputWithSourceFormatDescription:(CMFormatDescriptionRef)videoFormatDescription
    ...
    if (!videoSettings) {
        ...
        videoSettings = @{AVVideoCodecKey: AVVideoCodecTypeH264, AVVideoWidthKey: @(dimensions.width), AVVideoHeightKey: @(dimensions.height), AVVideoCompressionPropertiesKey: compressionProperties};
    }
    ...
    if ([_assetWriter canApplyOutputSettings:videoSettings forMediaType:AVMediaTypeVideo]) {
        ...
        if ([_assetWriter canAddInput:_videoInput]) {
            ...
            NSDictionary *sourcePixelBufferAttributesDictionary = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:kCVPixelFormatType_32BGRA],
                                                                                                             kCVPixelBufferPixelFormatTypeKey,
                                                                                                             [NSNumber numberWithInt:dimensions.width],
                                                                                                             kCVPixelBufferWidthKey,
                                                                                                             [NSNumber numberWithInt:dimensions.height],
...
+ (NSError *)cannotSetupInputError {
    ...
    NSDictionary *errorDict = @{NSLocalizedDescriptionKey: localizedDescription, NSLocalizedFailureReasonErrorKey: localizedFailureReason};
