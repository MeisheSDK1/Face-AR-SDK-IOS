...
- (instancetype)initWithFrame:(CGRect)frame glContext:(EAGLContext *)glContext {
...
- (void)commonInit;
{
    ...
    self.fillMode = EFPreviewFillModePreserveAspectRatio;
...
- (BOOL)setupLumaTextureWithPixelBuffer:(CVPixelBufferRef)pixelBuffer {
    ...
    if (pixelFormat == kCVPixelFormatType_32BGRA) {
        cvRet = CVOpenGLESTextureCacheCreateTextureFromImage(
            kCFAllocatorDefault, _cvTextureCache, pixelBuffer, NULL, GL_TEXTURE_2D, GL_RGBA, bufferWidth, bufferHeight, GL_BGRA, GL_UNSIGNED_BYTE, 0, &_cvCameraTexture);
        ...
        glActiveTexture(GL_TEXTURE0);
        cvRet = CVOpenGLESTextureCacheCreateTextureFromImage(
            kCFAllocatorDefault, _cvTextureCache, pixelBuffer, NULL, GL_TEXTURE_2D, GL_LUMINANCE, bufferWidth, bufferHeight, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0, &lumaTexture);
        if (cvRet) {
        ...
        cvRet = CVOpenGLESTextureCacheCreateTextureFromImage(
            kCFAllocatorDefault, _cvTextureCache, pixelBuffer, NULL, GL_TEXTURE_2D, GL_LUMINANCE_ALPHA, bufferWidth / 2, bufferHeight / 2, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, 1, &chromaTexture);
        if (cvRet) {
            NSLog(@"Error at CVOpenGLESTextureCacheCreateTextureFromImage %d", cvRet);
        } else {
